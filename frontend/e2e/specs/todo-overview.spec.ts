...
test.describe('Todo Overview', () => {
  test('should show correct statistics cards on overview page', async ({
    page,
    stateApi
  }) => {
    const todos = [
    ...
    await expect(todoPage.statsProgress.locator('span').first()).toHaveText(
      /^33%$/
    );
  });
  ...
  test('should navigate to tasks from view all link in overview', async ({
    page,
    stateApi
  }) => {
    const todos = [buildTodo({ title: 'Test Task' })];
    await stateApi.resetDatabase(todos);
