...
2. **Review existing:**
   - Check logger.log/logger.error usage
   - Review error handling in api procedures
   - Examine FetchError patterns

3. **Add structured logging (Rest):**
   - Log at entry: `logger.log("[procedure] Starting", {userId, input})`
   - Log success: `logger.log("[procedure] Success", {result})`
   - Log errors: `logger.error("[procedure] Failed", {error, context})`
   - Use FetchError with appropriate codes
