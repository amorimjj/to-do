...
4. **Verify test fails:**
   - Run: `npm run test {test-file}` or `npm run test:e2e {test-file}`
...
6. **Verify tests pass:**
   - Run test: `npm run test {test-file}` or `npm run test:e2e {test-file}`
...
8. **Run related tests:**
   - `npm run test {affected-directory}` or `npm run test`
